...
from ..train.pcb import PcbDataset

# class SbdTestDataset(Dataset):
#     pass

class PcbTestDataset(PcbDataset):
    ...
    def __getitem__(self, index):
        ...
        if 'pixel' in self.cfg.commen.task:
            # ✅ Pixel 태스크: contour 관련 num_points 설정 불필요
            pass
        elif 'snake' in self.cfg.commen.task:
        ...
        elif 'ccp_maskinit' in self.cfg.commen.task:
            # ccp_maskinit에서는 wh head를 사용하지 않음
            data_input.update({'num_points_init': self.cfg.commen.init_points_per_poly, 'num_points_coarse': self.cfg.commen.init_points_per_poly, 'num_points': self.cfg.data.points_per_poly})
        else:
            # ✅ wh head 존재 여부 확인
