...
class NetworkWrapper(nn.Module):
    def __init__(self, net, with_dml=True, ml_start_epoch=10, weight_dict=None, cfg=None):
        ...
        # self.reg_crit = IndL1Loss1d('smooth_l1')
    ...
    def compute_loss(self, output, batch, output_t=None, mode='default'):
        ...
        if 'ex' in self.weight_dict:
            weight_ex = self.weight_dict['ex']
        else:
            weight_ex = 1.
        loss += ex_loss * weight_ex

        # py
        py_loss = 0
        for i in range(len(output['py_pred'])):
            py_loss += self.py_crit(output['py_pred'][i], output['img_gt_polys']) / len(output['py_pred'])
        scalar_stats.update({'py_loss': py_loss})
        if 'py' in self.weight_dict:
            weight_py = self.weight_dict['py']
        else:
            weight_py = 1.
        loss += weight_py * py_loss
