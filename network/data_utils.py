...
def has_self_intersection(contour):
    ...
    return: bool (True if self-intersecting)
    """
    ...
    if len(contour.shape) != 2 or contour.shape[1] != 2:
        raise ValueError(f"Expected shape (V, 2), got {contour.shape}")

    if len(contour) < 3:
        return False  # can't self-intersect
    ...
    if torch.is_tensor(polygons):
        ...
        for poly in polygons:
            if torch.is_tensor(poly):
                poly = poly.cpu().numpy()  # make it easier to handle
            V = poly.shape[0]
            simple = True
            ...
            for i in range(V):
                ...
                for j in range(i + 1, V):
                    # skip adjacent edges or same edge
                    if j == (i + 1) % V or i == (j + 1) % V:
                        continue
        ...
        is_simple = torch.tensor(results, dtype=torch.bool)
...
def select_point(shape, poly, gt_mask, max_iter=3):
...
def box_iou(box1, box2):
    """box1: [n, 4], box2: [m, 4]"""
    area1 = get_area(box1) # n
    area2 = get_area(box2) # m

    lt = torch.max(box1[:, None, :2], box2[:, :2]) # (n, m)
    rb = torch.min(box1[:, None, 2:], box2[:, 2:]) # (n, m)
    wh = (rb - lt + 1).clamp(min=0) # (n, m)
    inter = wh[:, :, 0] * wh[:, :, 1] # (n, m)
...
def topk(scores, K=40):
    ...
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K) # top K for each batch x cat : (batch, cat, K)

    topk_inds = topk_inds % (height * width)
    topk_ys = (topk_inds / width).int().float()
    topk_xs = (topk_inds % width).int().float()

    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K) # top K for each batch : (batch, K)
    topk_clses = (topk_ind / K).int()
